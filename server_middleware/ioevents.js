...
var redisCleanup = require('./share_cleanup.js');
...
//	upon connection, check to see if the room exists..
			...
			function sendJoinAck(role){
				...
				socket.request.session.save(function(err){
					socket.join(socket.room, function(err){
						...
						socket.emit('joinsuccess', {role: role});
	  	...
	  	shareContainer.getClients(socket.room, function(err, val){
	  		if (val.indexOf(value) > -1){
	  			...
	  			shareContainer.setNickname(value, socket.request.session.shares[socket.room].nick, socket.request.sessionID, socket.room, function(err, val){
	  				var msg = {type: 'notice', content: socket.request.session.shares[socket.room].nick+ ' has changed nick to '+value};
	  				shareContainer.addMessage(msg, socket.room, function(msgID){
	  ...
	  socket.on('destroyroom', function(){
	  	shareContainer.getClientDetails(socket.request.session.shares[socket.room].nick, socket.room, function(val){
	  		if(val.role == 'owner'){
					...
					// console.log("WALAO");
					cleanupJobs.expireShare(socket.room, function(roomid){
						console.log(roomid);
						...
						// 	clientSocket.emit('new event', "Updates");
							...
							if (s.request.session.uploads && s.request.session.uploads[socket.room]){
								delete s.request.session.uploads[socket.room];	
							}
	  ...
	  socket.on('textMessage', function(msg){
	  	if (socket.request.session.shares[socket.room]){
		    ...
		    shareContainer.addMessage(message, socket.room, function(msgID){
